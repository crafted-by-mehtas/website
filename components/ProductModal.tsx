...
export default function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  // Parse orderFields into individual field names for display
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <style>{`@media (max-width: 600px) { .modal-grid { grid-template-columns: 1fr !important; } }`}</style>
    </div>
  );
}
